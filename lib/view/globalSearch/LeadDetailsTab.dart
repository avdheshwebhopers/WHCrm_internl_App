...
//       {Key? key, required this.data, required this.directoryPath})
//       : super(key: key);
//
//   @override
...
//         _showDialogBox(
//             phoneNumberToSearch, latestMp3FilePath, latestMp3FileName!);
//       }
...
//       String phoneNumber, Uint8List mp3FileData, String mp3FileName) async {
...
//                           remarkController.text = callTypeData.lead!.keys
//                               .firstWhere(
//                                   (key) => callTypeData.lead![key] == value,
//                                   orElse: () => ''); // Update remark field
//                         });
//                       },
...
//       _getCallLog(
//           "+91${lead['mobile']}"); // Fetch call logs after making a call
//     } catch (e) {
//       print('Error calling number: $e');
//     }
//   }
//
//   Future<void> _setCallDetails(
...
//     _leadDetailsViewModel.remark.value = remark.toString();
//     _leadDetailsViewModel.calltype.value = selectedcalltype.toString() ?? "";
//     print("notanswered: ${_leadDetailsViewModel.id.value.toString()} , "
//         "${_leadDetailsViewModel.type.value} , "
//         "${_leadDetailsViewModel.duration.value},"
//         " ${_leadDetailsViewModel.date.value} , "
//         "${_leadDetailsViewModel.fromNumber.value} ,"
//         " ${_leadDetailsViewModel.toNumber.value} , "
//         "${_leadDetailsViewModel.remark.value} ${selectedcalltype}");
//
...
class LeadDetailsTab extends StatefulWidget {
  ...
  const LeadDetailsTab({
    Key? key,
    required this.data,
    this.directoryPath, // Made nullable
  }) : super(key: key);
...
class _LeadDetailsTabState extends State<LeadDetailsTab> with WidgetsBindingObserver {
  ...
  Future<void> _updateSelectedLead(Map<String, dynamic> lead) async {
      _selectedLead = lead;
  }
  ...
  void _onAppResumed() async {
    if (_callDisconnected && _selectedLead != null) {
      final String phoneNumberToSearch = "+91" + _selectedLead!['mobile'];
        print(phoneNumberToSearch);
      await _getCallLog(phoneNumberToSearch);
      if (widget.directoryPath != null && widget.directoryPath!.isNotEmpty) {
        Uint8List latestMp3FilePath = await _getLatestMp3FileData();
        String? latestMp3FileName = await _getLatestMp3FileName();
        if (latestMp3FilePath.isNotEmpty && latestMp3FileName != null) {
          _showDialogBox(phoneNumberToSearch, latestMp3FilePath, latestMp3FileName);
        }
      } else {
        _showDialogBox(phoneNumberToSearch, Uint8List(0), null);
      }
    }
  }
  ...
  Future<void> _showDialogBox(String phoneNumber, Uint8List mp3FileData, String? mp3FileName) async {
    ...
    selectedOption = await showDialog(
      ...
      barrierDismissible: false,
      builder: (context) => AlertDialog(
        title: Text("ACTIVITY UPDATE", style: TextStyle(fontSize: 20)),
        content: SingleChildScrollView(
          child: StatefulBuilder(
            builder: (BuildContext context, StateSetter setState) {
              return Column(
                ...
                children: [
                  ...
                  GestureDetector(
                    ...
                    child: ListTile(
                      title: selectedDateTime == null
                          ? Text('Set Reminder', style: TextStyle(fontSize: 17))
                          : Text(' ${selectedDateTime.toString()}', style: TextStyle(fontSize: 15)),
                      trailing: Icon(Icons.calendar_today),
                    ),
                  ),
                  SizedBox(height: 10),
                  ...
                  if (callTypeData.lead != null)
                    DropdownButtonFormField<String>(
                      ...
                      onChanged: (String? value) {
                        setState(() {
                          selectedOption = value;
                          remarkController.text = callTypeData.lead!.keys.firstWhere(
                                  (key) => callTypeData.lead![key] == value,
                              orElse: () => '');
                        });
                      },
          ...
          ElevatedButton(
            onPressed: () {
              if (selectedOption != null) {
                ...
                _setCallDetails(phoneNumber, mp3FileData, text, selectedOption!, formattedDateTime, mp3FileName);
                Navigator.pop(context, selectedOption);
              } else {
                // Display an error message or handle incomplete form data
              }
  ...
  Future<String?> _getLatestMp3FileName() async {
    if (widget.directoryPath == null || widget.directoryPath!.isEmpty) {
      print('Directory path is empty or null');
      return null;
    }
    ...
    try {
      Directory directory = Directory(widget.directoryPath!);
      List<FileSystemEntity> files = directory.listSync(recursive: true);
      List<File> mp3Files = files.where((file) => file.path.toLowerCase().endsWith('.mp3')).map((file) => File(file.path)).toList();
  ...
  Future<void> _makeCall(Map<String, dynamic> lead) async {
    try {
      await FlutterPhoneDirectCaller.callNumber("+91${lead['mobile']}");
      await _updateSelectedLead(lead);
      // Fetch call logs after making a call
    } catch (e) {
      print('Error calling number: $e');
    }
  }
